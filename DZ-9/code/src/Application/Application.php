...
class Application
{
            ...
            if (method_exists($this->controllerName, $this->methodName)) {
                ...
                if ($controllerInstance instanceof AbstractController) {
                    if ($this->checkAccessToMethod($controllerInstance, $this->methodName)) {
                        ...
                        Application::logErrorToFile("Попытка доступа без прав к методу " . $this->methodName . " контроллера " . $controllerName . Application::commonTailOfLogMessage());
                ...
                Application::logErrorToFile("Попытка доступа к несуществующему методу " . $this->methodName . " контроллера " . $controllerName . Application::commonTailOfLogMessage());
                throw new Exception("Метод " .  $this->methodName . " не существует");
            }
        } else {
            Application::logErrorToFile("Попытка доступа к несуществующему контроллеру " . $controllerName . Application::commonTailOfLogMessage());
    ...
    {
        if ($methodName === 'actionAuth' || $methodName === 'actionLogin')
            return true;
