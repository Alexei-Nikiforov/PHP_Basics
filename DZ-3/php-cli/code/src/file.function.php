...
function addFunction(array $config) :string {
    ...
    if (!validateDate($date)) {
        return handleError("Некорректная дата рождения");
    }
    ...
    if (fwrite($fileHandler, $jsonEncodedData) === false) {
        fclose($fileHandler);
        return handleError("Произошла ошибка записи. Данные не сохранены. Попробуйте снова.");
    }
...
function searchBD(array $config) :string {
    ...
    return $result ?? 'На сегодня дней рождения нет';
}
...
function deleteUser(array $config) :string {
    ...
    if (is_readable($address)) {
        ...
        if (!$userFound) {
            return handleError("Пользователя нет");
        }
